...
import { insertAlertNotification, insertAuditLog } from "@/lib/server/repository";
import type { AlertNotificationSeverity, AlertNotificationStatus } from "@/types/dba";
...
function buildMetadata(item: BodyRecord, merged: BodyRecord) {
...
export async function POST(request: Request) {
  try {
    ...
    for (const [index, item] of items.entries()) {
      ...
      const db = readString(merged, ["db", "database", "db_name", "dbName"]);
      ...
      const utilizationPct = readNumber(merged, ["utilization_pct", "utilizationPct", "pct_used", "pctUsed", "used_pct", "usedPct"]);
      const thresholdPct = readNumber(merged, ["threshold_pct", "thresholdPct", "threshold"]) ?? 90;
      const criticalPct = readNumber(merged, ["critical_pct", "criticalPct", "critical_threshold_pct", "criticalThresholdPct"]) ?? thresholdPct;
      ...
      const actor = readString(merged, ["created_by", "createdBy", "requested_by", "requestedBy", "actor"]) || PUBLIC_ALERT_ACTOR;
      const incomingAlertId = readString(merged, ["id", "alert_id", "alertId"]);
      const alertId = incomingAlertId && items.length === 1 ? incomingAlertId : incomingAlertId ? `${incomingAlertId}-${index + 1}` : createAlertId(index + 1);
      ...
      const alert = await insertAlertNotification({
        ...
        usedGb,
        freeGb,
        workflowRunId: readString(merged, ["workflow_run_id", "workflowRunId", "execution_id", "executionId"]),
        callbackUrl: readString(merged, ["callback_url", "callbackUrl", "ack_callback_url", "ackCallbackUrl"]),
        createdBy: actor,
        metadata: buildMetadata(item, merged)
      });
